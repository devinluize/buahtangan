        ...
        <div class="grid grid-cols-3 gap-[3rem] mb-10">
            ...
            @foreach ($album as $a)
                <div class="flex flex-col border-[0.3rem] border-lightGrey p-12 w-[27rem] h-[31rem] shadow-md">
                    <img src="{{ asset('storage/' . $a->path) }}" alt="bayamku" class="w-[20rem] h-[20rem]">
                    <h2 class="mt-8 font-semibold text-[21px]">{{ $a->album_name }}</h2>
                    <p class="text-[14px]">{{ $a->created_at }}</p>
                </div>
                {{-- @endfor --}}
            @endforeach
